...
import DispTDays from "./DispTDays";
...
export default function DispTDaysCollection(props) {
    ...
    while ((newCache.length < cacheUntil || !isPaginated) && newNext != null) {
      ...
      const result = (
        await client.graphql({
          query: listDays.replaceAll("__typename", ""),
          variables,
        })
      ).data.listDays;
  ...
  return (
    <div>
      <Collection
        ...
        {...getOverrideProps(overrides, "DispTDaysCollection")}
        {...rest}
      >
        {(item, index) => {
          if (loading) {
